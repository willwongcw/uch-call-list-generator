...
def select_file_and_run():
    ...
    if excel_file_path:
        ...
        # After processing, get the directory and base filename
        # Determine the final path with a unique filename
        directory, original_filename = os.path.split(excel_file_path)
        base_filename = f'callList_{year}_{month}'
        file_number = 0
        while True:
            final_filename = f"{base_filename}-{file_number}.xlsx" if file_number else f"{base_filename}.xlsx"
            ...
            if not os.path.exists(final_path):
                ...
                with pd.ExcelWriter(final_path, engine='openpyxl') as writer:
                    ...
                    #copy expected metric to output
                    expected_metrics.to_excel(writer, sheet_name='Sheet1', startrow=3, header=True, index=False)

                    format_output_excel(writer, year, month, total_calls, total_sat_calls, total_sun_calls, three_day_calls, four_days_calls, five_day_calls, six_day_calls, seven_day_calls)  # Call the formatting function

                break
            file_number += 1
        
        # Show the output file location and name
        messagebox.showinfo("Success", f"Output saved to: {final_path}")
    else:
